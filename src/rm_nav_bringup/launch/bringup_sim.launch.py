...
def generate_launch_description():
      ...
      print(f"Debug: Checking source directory = {src_rm_nav_bringup_dir}")

      # 检查工作区目录是否存在
      print(f"Debug: Workspace exists? = {os.path.exists(workspace_dir)}")
      print(f"Debug: Source rm_nav_bringup directory exists? = {os.path.exists(src_rm_nav_bringup_dir)}")

      # 列出工作区目录内容
      if os.path.exists(workspace_dir):
          print(f"Debug: Workspace contents = {os.listdir(workspace_dir)}")

      # 列出src目录内容
      if os.path.exists(os.path.join(workspace_dir, 'src')):
          print(f"Debug: src directory contents = {os.listdir(os.path.join(workspace_dir, 'src'))}")

      # 列出rm_nav_bringup目录内容
      if os.path.exists(src_rm_nav_bringup_dir):
          print(f"Debug: rm_nav_bringup directory contents = {os.listdir(src_rm_nav_bringup_dir)}")

      # 列出config目录内容
      if os.path.exists(os.path.join(src_rm_nav_bringup_dir, 'config')):
          print(f"Debug: config directory contents = {os.listdir(os.path.join(src_rm_nav_bringup_dir, 'config'))}")

      # 列出simulation目录内容
      if os.path.exists(os.path.join(src_rm_nav_bringup_dir, 'config', 'simulation')):
          print(f"Debug: simulation directory contents = {os.listdir(os.path.join(src_rm_nav_bringup_dir, 'config', 'simulation'))}")

      # 测试源代码目录中的配置文件是否存在
      src_config_file = os.path.join(src_rm_nav_bringup_dir, 'config', 'simulation', 'nav2_params_intpc.yaml')
      print(f"Debug: Testing source config file at = {src_config_file}")
      print(f"Debug: Source file exists? = {os.path.exists(src_config_file)}")

      # 如果源代码目录中的配置文件存在，使用源代码目录
      if os.path.exists(src_config_file):
          rm_nav_bringup_dir = src_rm_nav_bringup_dir
          print(f"Debug: Using source directory")
      else:
          # 否则从安装目录获取
          rm_nav_bringup_dir = get_package_share_directory('rm_nav_bringup')
          print(f"Debug: Falling back to installed directory = {rm_nav_bringup_dir}")
          
          # 列出安装目录内容
          if os.path.exists(rm_nav_bringup_dir):
              print(f"Debug: Installed rm_nav_bringup directory contents = {os.listdir(rm_nav_bringup_dir)}")
          
          # 列出安装目录中的config目录内容
          if os.path.exists(os.path.join(rm_nav_bringup_dir, 'config')):
              print(f"Debug: Installed config directory contents = {os.listdir(os.path.join(rm_nav_bringup_dir, 'config'))}")
          
          # 列出安装目录中的simulation目录内容
          if os.path.exists(os.path.join(rm_nav_bringup_dir, 'config', 'simulation')):
              print(f"Debug: Installed simulation directory contents = {os.listdir(os.path.join(rm_nav_bringup_dir, 'config', 'simulation'))}")

      # 最终配置文件路径
      final_config_file = os.path.join(rm_nav_bringup_dir, 'config', 'simulation', 'nav2_params_intpc.yaml')
      print(f"Debug: Final config file path = {final_config_file}")
      print(f"Debug: Final file exists? = {os.path.exists(final_config_file)}")
      ...
      rm_nav_bringup_dir, 'config', 'simulation', 'measurement_params_sim.yaml')))
      robot_description = Command(['xacro ', os.path.join(
      rm_nav_bringup_dir, 'urdf', 'sentry_robot_sim.xacro'),
      ...
      start_localization_group_intpc = GroupAction(
            ...
            actions=[
                ...
                IncludeLaunchDescription(
                    ...
                    launch_arguments={
                        'use_sim_time': use_sim_time,
                        'params_file': intpc_params_file}.items()
                ),
      ...
      start_localization_group_teb = GroupAction(
            ...
            actions=[
                ...
                IncludeLaunchDescription(
                    ...
                    launch_arguments={
                        'use_sim_time': use_sim_time,
                        'params_file': default_params_file}.items()
                ),
